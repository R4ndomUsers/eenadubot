...
l = {f'Eenadu_TG {date}': 1, f'Eenadu_AP {date}': 2, f'Eenadu_GHYD {date}': 3}
# print('enter the quality you want')
quality = int(80)
# print(':-\n\t1. Main Papers\n\t2. District papers-AP\n\t3. District papers-TS\n\t4. Sunday Magzine')
ch = int(1)
#async def print_papers():


if (ch == 1):
    state =f'Eenadu_GHYD {date}'
    try:
        get_paper(dt[0], dt[1], dt[2], l[state], 0, state, 'Mains')
        print('your pdf is saved as '+state+'.pdf')
    except Exception as e:
        print(str(e))
        print('paper not found')
    state =f'Eenadu_AP {date}'
    try:
        bg1 = []
        over1 = []
        com = []

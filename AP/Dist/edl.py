...
def get_regions(m,n):
	with requests.Session() as s:
		regions=s.get('https://epaper.eenadu.net/Home/GetEditionListById?type='+str(m)+'&EditionId='+str(n),headers=headers)
...
def get_network(path):
	...
	try:
		...
		print('no internet')
		return 0
header1={'Referer': 'https://epaper.eenadu.net/Home/Index', 'Sec-Fetch-Mode': 'no-cors', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.100 Safari/537.36'}
...
def load_bg(url,no):
	...
	if ((co/total)==1):
		...
		print(' ','■'*int((co/total)*50),'  ',str(int((co/total)*100))+'%',end='\r')
...
def get_paper(d,m,y,n,z,edition_name,folder):
	global total
	with requests.Session() as s:
		r=s.get('https://epaper.eenadu.net/Home/GetAllpages?editionid='+str(n)+'&editiondate='+d+'%2F'+m+'%2F'+y,headers=headers)
		k=r.json()
	j=[]
	for i in k:
	...
	if (z!=0):
		with requests.Session() as s:
			r=s.get('https://epaper.eenadu.net/Home/GetAllpages?editionid='+str(z)+'&editiondate='+d+'%2F'+m+'%2F'+y,headers=headers)
			k=r.json()
		for i in k:
			j.append(i['XHighResolution'])
	...
	try:
		...
		pass
	with open(folder+"/"+edition_name+".pdf", "wb") as f:
